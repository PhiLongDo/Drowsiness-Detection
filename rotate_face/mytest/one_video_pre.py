...
# lay mau chop mat
blinksTXT = './drowsiness_data/alert.txt'
...
del(time_open,time_close,left_max,left_min,right_min,right_max,AVGmouth,AVGfaceAngleZs,AVGfaceAngleYs,AVGfaceAngleXs,num_blinks)
# --------------------------------------------------------------------

window_size =5
stride = 2
len_file = './data_preprocess/len_o1-3.txt'
Testing    = './data_preprocess/BlinksTest_o1-3_'+str(window_size)+'.npy'
blinksTXT ='./drowsiness_data/o1-3.txt'
lenx = 0
# xu ly file test
time_open = np.loadtxt(blinksTXT, usecols=0)
time_close = np.loadtxt(blinksTXT, usecols=1)
...
output = unroll_in_time(normalized_blinks, window_size, stride)
# labels = 2 * np.ones([len(output), 1])
lenx += len(output)
...
# LabelsTest = './data_preprocess/LabelsTest_o2_'+str(window_size)+'.npy'
