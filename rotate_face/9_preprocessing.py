...
def unroll_in_time(in_data, window_size, stride):
    ...
    n = len(in_data)
    if n <= window_size:
        out_data = np.zeros([1, window_size, 9])
        out_data[0, -n:, :] = in_data
        return out_data
    else:
        N = ((n - window_size) // stride) + 1
        out_data = np.zeros([N, window_size, 9])
        for i in range(N):
...
for window_size in [5,10,20,30]: 
    for i in [1,2,3,4,5]:
        ...
        Training   = './data_preprocess/9_Blinks_'+str(window_size)+'_'+test_fold+'.npy'
        LabelsTrain= './data_preprocess/9_Labels_'+str(window_size)+'_'+test_fold+'.npy'
        Testing    = './data_preprocess/9_BlinksTest_'+str(window_size)+'_'+test_fold+'.npy'
        LabelsTest = './data_preprocess/9_LabelsTest_'+str(window_size)+'_'+test_fold+'.npy'
        len_file = './data_preprocess/9_'+str(window_size)+'_'+test_fold+'.txt'
